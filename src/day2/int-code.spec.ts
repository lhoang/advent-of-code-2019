import {compute} from './int-code';

describe('Int Code', () => {
  it('should compute the given test commands', () => {
    expect(compute('1,0,0,0,99')).toEqual('2,0,0,0,99');
    expect(compute('2,3,0,3,99')).toEqual('2,3,0,6,99');
    expect(compute('2,4,4,5,99,0')).toEqual('2,4,4,5,99,9801');
    expect(compute('1,1,1,4,99,5,6,0,99')).toEqual('30,1,1,4,2,5,6,0,99');
    expect(compute('1,9,10,3,2,3,11,0,99,30,40,50')).toEqual('3500,9,10,70,2,3,11,0,99,30,40,50');
  });

  it('should compute the right result before 1202 program alarm', () => {
    // With corrected values
    const input = '1,12,2,3,1,1,2,3,1,3,4,3,1,5,0,3,2,10,1,19,1,19,5,23,1,23,9,27,2,27,6,31,1,31,6,35,2,35,9,39,1,6,39,' +
      '43,2,10,43,47,1,47,9,51,1,51,6,55,1,55,6,59,2,59,10,63,1,6,63,67,2,6,67,71,1,71,5,75,2,13,75,79,1,10,79,83,1,5,' +
      '83,87,2,87,10,91,1,5,91,95,2,95,6,99,1,99,6,103,2,103,6,107,2,107,9,111,1,111,5,115,1,115,6,119,2,6,119,123,1,5,' +
      '123,127,1,127,13,131,1,2,131,135,1,135,10,0,99,2,14,0,0';

    const res = compute(input).split(',')[0];

    expect(res).toEqual(4945026);
  });

});